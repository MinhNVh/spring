    ...
    stages {
        ...
        stage('Deploy MySQL to DEV') {
            steps {
                ...
                sh "docker run --name minh-mysql --rm --network dev -v khalid-mysql-data:/var/lib/mysql -e MYSQL_ROOT_PASSWORD=${MYSQL_ROOT_LOGIN_PSW} -e MYSQL_DATABASE=db_example  -d mysql:8.0 "
                sh 'sleep 20'
